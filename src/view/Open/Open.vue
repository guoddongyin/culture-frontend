<template>
    <div id="home">
        ...
        <div class="homeCenter" style="margin-bottom: 0">
            ...
            <div style="padding-top: 2.8rem;overflow: auto">
                <div @click="handleLink(item)" v-for="(item,index) in contents" :key="index" class="Accontent"
                     style="border-bottom:none" :style="index===0?'margin-top:0':''">
                    <div class="images">
                        <img v-lazy="url+item.img" alt="">
                        <span>{{ item.waitState | IsEnroll }}</span>
                    </div>
...
<script>
    import { areas, restype, basecode, freeItems, allFreeItems, freeItemKey } from '../../api/api'
    ...
    export default {
        ...
        data() {
            return {
                ...
                typeId: '',                     // 类型ID，用于请求数据
                // 时间选择数组
                pickTime: ['正在报名', '即将开班', '还未开班', '历史开班', '选择日期'],
                todayStartTime: '',
                todayEndTime: '',
                tomorrowStartTime: '',
                tomorrowEndTime: '',
        ...
        methods: {
          selectDate(date){
            ...
            // 搜索框搜索
            handleKey() {
                freeItemKey(this.searchText).then(res => {
            ...
            clickMore() {
              this.offset=this.offset+this.limit
                if (this.total > this.contents.length) {
                    ...
                    freeItems(this.areaId, this.startTime, this.endTime, this.typeId, this.offset, this.limit, this.allCount).then(res => {
            ...
            getData() {
                Indicator.open()
                this.offset = 0
              freeItems(this.areaId, this.startTime, this.endTime, this.typeId, this.offset, this.limit, this.allCount,this.enrollTime).then(res => {
                    // 取出在头部的总条数
                ...
                if (IS_WX) {
                  ...
                  freeItems(this.areaId, this.startTime, this.endTime, this.typeId, this.offset, this.limit, this.allCount,this.enrollTime).then(res => {
                    ...
                    JsBridge.callHandler('getArea', '', (data) => {
                        ...
                        this.offset = 0
                        allFreeItems(this.limit, this.allCount, this.areaId).then(res => {
                            // 取出在头部的总条数
                            this.total = res.count
                            res.data.map(m => m.type = m.type.split(','))
                            this.contents = res.data
...
//                this.areaId = item.id
          ...
          hanelActiName(item, index) {
            ...
            if (index === 0) {
              ...
              this.endTime = this.getTimeStamp(15);
              this.enrollTime='';
            } else if (index === 2) {
              this.startTime = this.getTimeStamp(0);
              this.endTime = ''

        ...
        <mu-list>
            <div class="appointmentOrder" v-for="(item,index) in allList" :key="index">
                ...
                <div class="flexBettwens">
                    ...
                    <div @click="handleOrder(item)" class="insideatBtn">查看</div>
                </div>
            </div>
        </mu-list>
    ...
    export default {
        ...
        data() {
            return {
                ...
                refreshing: false,
                trigger: null,
                state:9,                // 状态:0暂存与退回、1活动接收与配送、2活动确认和评论、9全部
            }
        },
        methods: {
            // 获取总数
            getCount(){
                waitcount().then(res=>{
                    console.log(res)
                    res.map(m=>m.typdid==='9'?this.total=m.count:'')
                    this.getData()
            ...
            handleOrder(item){
                isAll=true;
                if(item.status==='1'){
                    changeViewApply(item.id).then(res=>{
                        console.log(res)

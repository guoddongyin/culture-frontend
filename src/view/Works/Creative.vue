        ...
        <div class="homeCenter" style="margin-bottom: 0rem">
            ...
            <div style="padding-top: 1rem;overflow: auto;background-color: #e5e5e5">
                <div @click="handleLink(item)"
                     ...
                     :key="index"
                     v-if="isState===''?2>1:isState===item.waitState"
                     class="Accontent first"
                     :style="index===0?'margin-top:0':'margin-top:0.2rem'"
                     style="padding-top: 0">
                    <div class="images" style="margin-bottom: 0">
                        <img v-lazy="url+item.inageUrl" alt="">
                        <span>{{ item.waitState | state }}</span>
                        <div style="padding:0.4rem;border-bottom: 1px solid #e5e5e5">
                            <div style="font-size:0.4rem;" class="ell">
                                {{ item.subject }}
                            </div>
                        </div>
                    </div>
                    <div class="venue" style="border: none">
                        <div>{{ item.createrOrgName }}</div>
                        <div>征集时间：{{ item.startTime | times }} 至 {{ item.endTime | times }}</div>
                    </div>
                </div>
...
<script>
    import { works, frontArea, areas } from '../../api/api'
    ...
    export default {
        ...
        data() {
            return {
                ...
                areaId: '',                      // 区域ID--用于请求数据
                // 征集状态
                isSolicitation: '活动状态',       // 征集状态
                typePop: false,                  // 征集状态控制器
                pickState: ['状态不限', '正在进行', '已经结束'],
	            isState: '',                     // 状态代码
        ...
        methods: {
            ...
            clickMore() {
              this.offset=this.offset+this.limit
                if (this.total > this.contents.length) {
                    ...
                    frontArea(this.areaId, this.offset, this.limit, this.allCount,this.isState).then(res => {
                        console.log(res)
                        let arr = res.data
            ...
            getFirstData() {
                ...
                if (IS_WX) {
                    ...
                    this.offset = 0
                    works(this.areaId,this.limit,this.offset,this.allCount).then(res => {
                        console.log(res)
                        // 取出在头部的总条数
                        let headers = JSON.stringify(res.headers).replace(/-/g, '_')
                        this.total = JSON.parse(headers).x_total_count
...
//                this.areaId = item.id
            ...
            // 跳转到详情页
            handleLink(item) {
                this.$router.push('/Creatives/' + item.oid)
            }
        },
        created() {
            this.getArea()
